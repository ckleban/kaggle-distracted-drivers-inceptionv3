# Kleban's Notes
#
# This file does the retraining. Run it to create a new model
# based on the driver training data.
#
#
...
#
# - Added support for splitting up the train/CV/test data sets based on drivers
# - Added support to increase the number of images to train up by doing some
#   image distrotion via the process_image.sh script.
#
#
#
# ==============================================================================
...
tf.app.flags.DEFINE_string(
    'bottleneck_dir', '/Users/ChrisKleban/tensorflow-retrain/bottleneck',
...
###############################################################
#######  kleban's new variables

# Specify what drivers to put in the CV and test data sets.

driversToPutInCV = []
driversToPutInCV.append('p081')
driversToPutInCV.append('p012')
driversToPutInCV.append('p002')

driversToPutInTesting = []
driversToPutInTesting.append('p072')
driversToPutInTesting.append('p039')
driversToPutInTesting.append('p041')
driversToPutInTesting.append('p042')

# If you use the process_images.sh file to create more train data, set this to true
# so that the file can read the new files correctly and map them to drivers.

tf.app.flags.DEFINE_boolean(
    'usingProcessed', False,
    """If using preprocessed images, set to true in order to match drivrs to images correctly """)
usingProcessed = FLAGS.usingProcessed

SEED = 66478 # Set to None for random seed.

# I was playing with adding dropout to the retraining step. Didn't seem to help so this VAR is ignored for now.
tf.app.flags.DEFINE_float('dropout_rate', 0.5,
                          """what the dropout rate will be on the final layer.""")

####### End kleban's new variables
###############################################################



def create_image_lists(image_dir, testing_percentage, validation_percentage, imagesTodrivers, driversToPutInTesting, driversToPutInCV, usingProcessed):
  ...
  for sub_dir in sub_dirs:
    ...
    for file_name in file_list:
      ...
      ''' ### New way is to split by driver '''
      if usingProcessed == True:
          check_name = base_name[5:]
      else:
          check_name = base_name


      if (imagesTodrivers[check_name] in driversToPutInTesting):
        testing_images.append(base_name)
      elif (imagesTodrivers[check_name] in driversToPutInCV):
        validation_images.append(base_name)
      else:
        training_images.append(base_name)
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  ## Kleban trying to add dropout to retrain. This didn't seem to improve
  ## over fitting, so I'm dropping it out for now.

  #final_tensor = tf.nn.dropout(final_tensor, FLAGS.dropout_rate, seed=SEED)
